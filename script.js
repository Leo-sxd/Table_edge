...
class AIWebsiteController {
    ...
    async handleControl() {
        try {
            ...
            // 首先尝试直接执行本地指令
            const directResult = this.executeLocalCommand(command);
            if (directResult) {
                console.log('[AIControl] 本地指令直接执行成功');
                this.showStatus('指令已执行: ' + command, 'success');
                // 清空输入
                const input = document.getElementById('ai-control-input');
            ...
            if (localResult) {
                console.log('[AIControl] 本地解析成功，执行代码:', localResult);
...
class ScheduleImporter {
    ...
    init() {
        this.bindEvents();
        console.log('[ScheduleImporter] 课程表导入系统初始化完成');
    }
    ...
    async processHTML(file) {
        ...
        try {
            ...
            console.log('[ScheduleImporter] 文件大小:', text.length);
            ...
            // 提取课程数据
            const courses = this.extractCourses(doc);
            
            if (courses.length > 0) {
                // 显示提取结果
                this.showExtractionResult(courses);
                ...
                if (resultDiv) {
                    resultDiv.innerHTML = `
                        ...
                        未识别到课程数据<br>
                        <small>请确保上传的是正方教务系统的个人课表页面</small>
                    `;
            ...
            console.error('[ScheduleImporter] 解析失败:', error);
            if (resultDiv) {
                resultDiv.innerHTML = `<i class="fas fa-times-circle"></i> 解析失败: ${error.message}`;
    ...
    // 提取课程数据
    extractCourses(doc) {
        console.log('[ScheduleImporter] 开始提取课程...');
        
        // 策略1: 正方教务系统专用解析
        let courses = this.parseZhengfangSchedule(doc);
        
        // 策略2: 通用表格解析（如果专用解析失败）
        if (courses.length === 0) {
            courses = this.parseGenericSchedule(doc);
        }
        
        // 数据验证和清洗
        courses = this.validateCourses(courses);
        
        console.log(`[ScheduleImporter] 共提取 ${courses.length} 门课程`);
        return courses;
    }
    
    // 正方教务系统专用解析
    parseZhengfangSchedule(doc) {
        const courses = [];
        
        // 查找课表表格（正方教务系统通常使用 #kbgrid_table_0）
        const table = doc.querySelector('#kbgrid_table_0') || doc.querySelector('#Table1');
        if (!table) {
            console.log('[ScheduleImporter] 未找到正方教务表格');
            return courses;
        }
        
        console.log('[ScheduleImporter] 找到正方教务表格');
        
        const rows = table.querySelectorAll('tr');
        console.log('[ScheduleImporter] 表格行数:', rows.length);
        
        // 列映射：表格列索引 -> 星期几（1=周一，7=周日）
        const colToDay = { 2: 1, 3: 2, 4: 3, 5: 4, 6: 5, 7: 6, 8: 7 };
        
        // 时间段映射：节次 -> 时间段编号
        // 第1-2节 -> 1, 第3-4节 -> 2, 第5-6节 -> 3, 第7-8节 -> 4, 第9-10节 -> 5
        const getTimeSlot = (period) => Math.ceil(period / 2);
        ...
        for (let rowIdx = 2; rowIdx < rows.length; rowIdx++) {
            ...
            // 处理rowspan延续
            const processedCells = this.processRowspan(cells, rowspanTracker, rowIdx);
            
            // 提取节次信息
            const periodCell = processedCells[1];
            if (!periodCell || !periodCell.element) continue;
            
            const periodText = periodCell.element.textContent.trim();
            ...
            for (let colIdx = 2; colIdx <= 8 && colIdx < processedCells.length; colIdx++) {
                ...
                // 解析课程信息
                const cellCourses = this.parseCourseCell(content, dayOfWeek, timeSlot);
    ...
    // 处理rowspan延续
    processRowspan(cells, rowspanTracker, rowIdx) {
        const processed = [];
        let cellIdx = 0;
        ...
        for (let colIdx = 0; colIdx < 9; colIdx++) {
            if (rowspanTracker[colIdx] && rowspanTracker[colIdx].remaining > 0) {
                processed.push({
                    isContinuation: true,
                    course: rowspanTracker[colIdx].course
                });
                ...
                processed.push({ element: null, isContinuation: false });
            }
        }
    ...
    extractCellContent(cell) {
        ...
        // 排除空内容和无效内容
        if (!content || content === '' || content === '\xa0') return null;
        if (['上午', '下午', '晚上'].includes(content)) return null;
        
        return content;
    }
    
    // 解析课程单元格
    parseCourseCell(content, dayOfWeek, timeSlot) {
        const courses = [];
    ...
    // 解析单个课程块
    parseCourseBlock(block, dayOfWeek, timeSlot) {
        const lines = block.split('\n').map(l => l.trim()).filter(l => l.length > 0);
        if (lines.length === 0) return null;
        
        console.log('[ScheduleImporter] 解析课程块:', lines);
        
        // 提取课程名
        const courseName = this.extractCourseName(lines);
        ...
        const course = {
            ...
            day: dayOfWeek,
            time: timeSlot,
            location: this.extractLocation(detailText),
            startWeek: 1,
            endWeek: 20,
            weekType: ''
        };
        ...
        // 提取单双周
        if (detailText.includes('单周')) course.weekType = '单周';
        else if (detailText.includes('双周')) course.weekType = '双周';
        
        console.log('[ScheduleImporter] 识别课程:', course);
        return course;
    }
    ...
    extractCourseName(lines) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.includes('★') || line.includes('☆') || line.includes('◆')) {
                ...
                if (this.isValidCourseName(name)) return name;
                
                // 检查下一行
                if (i + 1 < lines.length) {
                    const nextLine = lines[i + 1].trim();
                    if (this.isValidCourseName(nextLine)) return nextLine;
                }
            }
        }
        ...
        for (const line of lines) {
            if (this.isValidCourseName(line)) return line;
        }
    ...
    isValidCourseName(name) {
        ...
        // 排除纯校区名称
        const campuses = ['骊山校园', '雁塔校园', '秦汉校园'];
        if (campuses.includes(name.trim())) return false;
        
        // 排除详情信息
        const keywords = ['周数', '校区', '地点', '教师', '学分', '考核', '节/周', '上午', '下午', '晚上'];
    ...
    extractLocation(text) {
        // 提取教室代码（如：2-1-402）
        const roomMatch = text.match(/(\d+-\d+-\d+|体育馆|2-机房|未排地点)/);
    ...
    parseGenericSchedule(doc) {
        ...
        for (const table of tables) {
            ...
            // 检查是否可能是课表
            if (text.includes('周一') && text.includes('周二') && 
                (text.includes('节') || text.includes('课程'))) {
                // 尝试通用解析
                // ... 实现通用解析逻辑
            }
        }
    ...
    validateCourses(courses) {
        ...
        for (const course of courses) {
            // 基本验证
            if (!course.name || course.name.length < 2) continue;
            if (!course.day || course.day < 1 || course.day > 7) continue;
            if (!course.time || course.time < 1 || course.time > 5) continue;
            ...
            // 数据修正
            if (!course.startWeek || course.startWeek < 1) course.startWeek = 1;
            if (!course.endWeek || course.endWeek > 20) course.endWeek = 20;
            if (course.startWeek > course.endWeek) {
                [course.startWeek, course.endWeek] = [course.endWeek, course.startWeek];
            }
    ...
    // 显示提取结果
    showExtractionResult(courses) {
        const textarea = document.getElementById('import-textarea');
        if (!textarea) return;
        
        const dayNames = ['', '周一', '周二', '周三', '周四', '周五', '周六', '周日'];
        const periodNames = { 1: '第1-2节', 2: '第3-4节', 3: '第5-6节', 4: '第7-8节', 5: '第9-10节' };
        
        const formatted = courses.map(c => {
            let line = c.name;
            line += ` ${dayNames[c.day]}`;
            line += ` ${periodNames[c.time] || `第${c.time}节`}`;
            if (c.location) line += ` ${c.location}`;
            if (c.startWeek !== 1 || c.endWeek !== 20) {
                line += ` ${c.startWeek}-${c.endWeek}周`;
                if (c.weekType) line += `(${c.weekType})`;
            }
            return line;
        }).join('\n');
        
        textarea.value = formatted;
        console.log('[ScheduleImporter] 已填充到文本框');
    }
}
...
class FavoritesModule {
    constructor() {
        ...
        this.btn = document.getElementById('favorites-btn');
        this.exitBtn = document.getElementById('favorites-exit-btn');
        this.bookmarksArea = document.getElementById('fav-bookmarks-area');
    ...
    getBookmarksData() {
        return {
            'website': [
                { name: 'Bing浏览器', url: 'https://cn.bing.com/', icon: '🔍', desc: '微软搜索引擎' },
                { name: '谷歌浏览器', url: 'https://www.google.com/', icon: '🔍', desc: '需要VPN' }
            ],
            'ai': [
                { name: 'ChatGPT', url: 'https://chat.openai.com/', icon: '🤖', desc: 'OpenAI对话AI' },
                { name: 'Claude', url: 'https://claude.ai/', icon: '🤖', desc: 'Anthropic对话AI' },
                { name: '文心一言', url: 'https://yiyan.baidu.com/', icon: '🤖', desc: '百度对话AI' },
                { name: '通义千问', url: 'https://tongyi.aliyun.com/', icon: '🤖', desc: '阿里对话AI' }
            ],
            'Agent': [
                { name: 'AutoGPT', url: 'https://autogpt.net/', icon: '🎯', desc: '自主AI代理' },
                { name: 'BabyAGI', url: 'https://github.com/yoheinakajima/babyagi', icon: '🎯', desc: '任务驱动AI' }
            ],
            'API': [
                { name: 'OpenAI API', url: 'https://platform.openai.com/', icon: '🔌', desc: 'OpenAI API平台' },
                { name: 'Hugging Face', url: 'https://huggingface.co/', icon: '🤗', desc: 'AI模型社区' }
            ],
            'programming': [
                { name: 'GitHub', url: 'https://github.com/', icon: '💻', desc: '代码托管平台' },
                { name: 'Stack Overflow', url: 'https://stackoverflow.com/', icon: '💻', desc: '开发者问答' },
                { name: 'MDN Web Docs', url: 'https://developer.mozilla.org/', icon: '📚', desc: 'Web开发文档' }
            ],
            'information': [
                { name: '维基百科', url: 'https://zh.wikipedia.org/', icon: '📖', desc: '百科全书' },
                { name: '知乎', url: 'https://www.zhihu.com/', icon: '❓', desc: '问答社区' }
            ],
            'academic': [
                { name: 'Google Scholar', url: 'https://scholar.google.com/', icon: '🎓', desc: '学术搜索' },
                { name: '知网', url: 'https://www.cnki.net/', icon: '🎓', desc: '中文学术资源' }
            ],
            'mailbox': [
                { name: 'Gmail', url: 'https://gmail.com/', icon: '📧', desc: 'Google邮箱' },
                { name: 'Outlook', url: 'https://outlook.live.com/', icon: '📧', desc: '微软邮箱' }
            ],
            'education': [
                { name: 'Coursera', url: 'https://www.coursera.org/', icon: '🎓', desc: '在线课程' },
                { name: '中国大学MOOC', url: 'https://www.icourse163.org/', icon: '🎓', desc: '中文慕课' }
            ],
            'english': [
                { name: 'Duolingo', url: 'https://www.duolingo.com/', icon: '🌐', desc: '语言学习' },
                { name: 'BBC Learning English', url: 'https://www.bbc.co.uk/learningenglish/', icon: '📰', desc: '英语学习' }
            ],
            'tools': [
                { name: 'Wolfram Alpha', url: 'https://www.wolframalpha.com/', icon: '🔬', desc: '计算知识引擎' },
                { name: 'Desmos', url: 'https://www.desmos.com/', icon: '📊', desc: '图形计算器' }
            ],
            'maps': [
                { name: 'Google Maps', url: 'https://maps.google.com/', icon: '🗺️', desc: '谷歌地图' },
                { name: '百度地图', url: 'https://map.baidu.com/', icon: '🗺️', desc: '百度地图' }
            ],
            'finance': [
                { name: '东方财富', url: 'https://www.eastmoney.com/', icon: '💰', desc: '财经资讯' },
                { name: '雪球', url: 'https://xueqiu.com/', icon: '💰', desc: '投资理财' }
            ],
            'computer': [
                { name: '系统设置', url: 'ms-settings:', icon: '⚙️', desc: 'Windows设置' },
                { name: '控制面板', url: 'control panel', icon: '🖥️', desc: '系统控制' }
            ],
            'web': [
                { name: 'CodePen', url: 'https://codepen.io/', icon: '✏️', desc: '前端代码编辑' },
                { name: 'JSFiddle', url: 'https://jsfiddle.net/', icon: '✏️', desc: '在线代码测试' }
            ],
            'games': [
                { name: 'Steam', url: 'https://store.steampowered.com/', icon: '🎮', desc: '游戏平台' },
                { name: 'itch.io', url: 'https://itch.io/', icon: '🎮', desc: '独立游戏' }
            ],
            'administration': [
                { name: '学校官网', url: '#', icon: '🏫', desc: '西南科技大学' },
                { name: '教务系统', url: '#', icon: '📋', desc: '学生服务' }
            ]
        };
    }
    
    loadBookmarks() {
        if (!this.bookmarksArea) return;
        ...
        if (this.currentCategory === 'all') {
            // 显示所有分类
            for (const [category, bookmarks] of Object.entries(data)) {
                if (bookmarks.length > 0) {
                    html += this.createCategorySection(category, bookmarks);
                }
            }
        } else {
            // 显示特定分类
            const bookmarks = data[this.currentCategory] || [];
            html = this.createBookmarksGrid(bookmarks);
        }
        
        this.bookmarksArea.innerHTML = html;
    }
    ...
    createCategorySection(category, bookmarks) {
        ...
        return `
            <div class="fav-category-section">
                <h3 class="fav-category-title">${categoryNames[category] || category}</h3>
                ${this.createBookmarksGrid(bookmarks)}
            </div>
        `;
    }
    
    createBookmarksGrid(bookmarks) {
        return `
            <div class="fav-bookmarks-area">
                ${bookmarks.map(bm => `
                    <a href="${bm.url}" class="fav-bookmark" target="_blank">
                        <div class="fav-bookmark-icon">${bm.icon}</div>
                        <div class="fav-bookmark-info">
                            <div class="fav-bookmark-name">${bm.name}</div>
                            <div class="fav-bookmark-url">${bm.desc}</div>
                        </div>
                    </a>
                `).join('')}
            </div>
    ...
    filterBookmarks(query) {
        ...
        for (const [category, bookmarks] of Object.entries(data)) {
            const filtered = bookmarks.filter(bm => 
                bm.name.toLowerCase().includes(query.toLowerCase()) ||
                bm.desc.toLowerCase().includes(query.toLowerCase())
            );
            
            if (filtered.length > 0) {
                html += this.createCategorySection(category, filtered);
            }
        }
        
        this.bookmarksArea.innerHTML = html || '<p style="text-align: center; color: #a0a0ff;">未找到匹配的书签</p>';
    }
    ...
    updateTime() {
        ...
        const timeEl = document.getElementById('fav-current-time');
        const periodEl = document.getElementById('fav-time-period');
        if (timeEl) timeEl.textContent = `${hours}:${minutes}:${seconds}`;
        if (periodEl) periodEl.textContent = period;
        
        // 日期
        const options = { year: 'numeric', month: 'long', day: 'numeric', weekday: 'long' };
        const dateEl = document.getElementById('fav-current-date');
        if (dateEl) dateEl.textContent = now.toLocaleDateString('zh-CN', options);
        
        // 农历（简化版）
        const lunarEl = document.getElementById('fav-lunar-date');
        if (lunarEl) {
            ...
            lunarEl.textContent = `农历${lunarMonths[month]}月${lunarDays[day % 30]}`;
        }
        
        // 节日
        const festivalEl = document.getElementById('fav-festival-info');
        if (festivalEl) {
            const month = now.getMonth() + 1;
            const day = now.getDate();
            const key = `${String(month).padStart(2, '0')}${String(day).padStart(2, '0')}`;
            const festivals = {
                '0101': '元旦', '0214': '情人节', '0308': '妇女节', '0401': '愚人节',
                '0501': '劳动节', '0601': '儿童节', '0701': '建党节', '0801': '建军节',
                '0910': '教师节', '1001': '国庆节', '1225': '圣诞节'
            };
            festivalEl.textContent = festivals[key] || '';
        }
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('[Favorites] 收藏功能模块已初始化');
});
